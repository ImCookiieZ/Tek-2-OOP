...
void Nibbler::start_game()
{
    ...
    //lib->draw(&_gameover_screen2, 20, 20, 28, 28);
...
std::string Nibbler::game_tick()
{
    ...
    if (elapsed > 200000 - 200000 * _fast - score * 1000)
    {
        ...
        lib->clear_screen();
//
//        object_creation_data s;
//        s.path_to_resource = "assets/block.png";
//        s.type = SPRITE;
//        lib->draw(lib->createObject(&s), 0, 1000, 0, 1000);
        draw_boarders();
...
void Nibbler::col_end()
{
    for (auto object = _snake.begin(); object != _snake.end(); object++)
    {
        ...
        boarder
        .second.x2 || object->second.y1 <= boarder.second.y1 || object->second
        .y1 >=
        boarder.second.y2)
        {
            _isRunning = false;
    ...
    if (!_isRunning)
    {
        ...
        for (auto &sc : _scores)
        {
            ...
            {
                os << _name.first.text.substr(6, _name.first.text.size() - 6) <<
                " " <<
                score <<
                std::endl;
                counter++;
